            ...
            <div class="ibox-content">
                <div class="row">
                    <div class="col-sm-4 form-inline">
                        <?= $this->tag->form(['admin/finance/export/recommend', 'method' => 'post', 'autocomplete' => 'on']) ?>
                    ...
                    <div class="col-sm-1">
                        <?= Wdxr\Modules\Admin\Tags\MenuTags::acl_button(['admin/finance/all_recommend', '导出全部推荐关系', 'class' => 'btn btn btn-default pull-right']); ?>
                    </div>
                </div>
                <table class="table table-hover">
                    ...
                    <?php if (isset($page->items)) { ?>
                        <?php foreach ($page->items as $recommend) { ?>
                            <tr>
                                <td><?= $recommend->id ?></td>
                                ...
                                <td><font color="red"><?php if ($recommend->status == 1) { ?><font color="green">正常</font><?php } elseif ($recommend->status == 2) { ?>票据异常<?php } elseif ($recommend->status == 3) { ?>征信异常<?php } elseif ($recommend->status == 4) { ?>企业信息异常<?php } elseif ($recommend->status == 5) { ?>企业服务期限异常<?php } else { ?>数据异常<?php } ?></font></td>
                                <td><?= $recommend->info ?></td>
                                <td><?= Wdxr\Modules\Admin\Tags\MenuTags::acl_button(['admin/finance/recommend_info/', '点击查看', 'href' => 'javascript:recommend_info(' . $recommend->byid . ')', 'class' => 'btn btn-primary btn-xs']); ?></td>
                ...
                <div class="row" >
                    ...
                    <div class="col-sm-7">
                        <ul class="pagination pull-right no-margins">
                            <li>
                                <?= $this->tag->linkTo(['admin/finance/recommend?page=1' . '&company_id=' . $company_id, '第一页']) ?>
                            </li>
                            <li class="paginate_button previous">
                                <?php if ($company_id) { ?>
                                    <?= $this->tag->linkTo(['admin/finance/recommend?page=' . $page->before . '&company_id=' . $company_id, '前一页']) ?>
                                <?php } else { ?>
                                    <?= $this->tag->linkTo(['admin/finance/recommend?page=' . $page->before, '前一页']) ?>
                                <?php } ?>
                            </li>
                            <li class="paginate_button next">
                                <?php if ($company_id) { ?>
                                    <?= $this->tag->linkTo(['admin/finance/recommend?page=' . $page->next . '&company_id=' . $company_id, '下一页']) ?>
                                <?php } else { ?>
                                    <?= $this->tag->linkTo(['admin/finance/recommend?page=' . $page->next, '下一页']) ?>
                                <?php } ?>
                            </li>
                            <li>
                                <?php if ($company_id) { ?>
                                    <?= $this->tag->linkTo(['admin/finance/recommend?page=' . $page->last . '&company_id=' . $company_id, '最后一页']) ?>
                                <?php } else { ?>
                                    <?= $this->tag->linkTo(['admin/finance/recommend?page=' . $page->last, '最后一页']) ?>
                                <?php } ?>
                            </li>
                        </ul>
                    </div>
...
<script type="text/javascript">
    function recommend_info(id) {
        $.ajax({
            type : "POST",
            url : "<?= $this->url->get('admin/finance/recommend_info/') ?>",
            data : {id:id},
