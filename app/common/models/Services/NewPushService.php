...
use XingeApp\Click;
use XingeApp\XingeApp;
use XingeApp\Message;
use XingeApp\Style;
use XingeApp\MessageIOS;
use XingeApp\TagTokenPair;
use XingeApp\TimeInterval;

class NewPushService extends Services

{

    const ACCESS_ID_ANDROID = '2100269595';
    const ACCESS_ID_KEY_ANDROID = 'A7UWJE1T946G';
    const SECRET_KEY_ANDROID = '5b9cd54c25d8313a2fba26db75cc08cc';

    const ACCESS_ID_IOS = '2200269594';
    const ACCESS_ID_KEY_IOS = 'I8H8JE6U8X1G';
    const SECRET_KEY_IOS = '1172e95e2e0e489633a52dc7a00b0f25';
...
//单个设备下发通知消息
    function DemoPushSingleDeviceNotification($content,$device_token)
    {
        $push = new XingeApp(self::ACCESS_ID_ANDROID, self::SECRET_KEY_ANDROID);
        $mess = new Message();
        $mess->setType(Message::TYPE_NOTIFICATION);
        $mess->setTitle($content['title']);
        $mess->setContent($content['body']);
        $mess->setExpireTime(86400);
        //$style = new Style(0);
        #含义：样式编号0，响铃，震动，不可从通知栏清除，不影响先前通知
        $style = new Style(0, 1, 1, 0, 0);
        $action = new Click();
        $action->setActionType(Click::TYPE_URL);
...
//下发IOS设备消息
    function DemoPushSingleDeviceIOS($content,$device_token,$num = 0)
    {
        $push = new XingeApp(self::ACCESS_ID_IOS, self::SECRET_KEY_IOS);
        $mess = new MessageIOS();
        $mess->setExpireTime(86400);
...
//        $mess->setAlert("ios test");
        ...
        $custom = array('num' => $num, 'key2' => 'value2');
        $mess->setCustom($custom);
        $acceptTime = new TimeInterval(0, 0, 23, 59);
        $mess->addAcceptTime($acceptTime);
        $raw = '{"xg_max_payload":1,"accept_time":[{"start":{"hour":"0","min":"0"},"end":{"hour":"23","min":"59"}}],"aps":{"alert":"'.$content['body'].'","title":"'.$content['title'].'","body":"'.$content['body'].'"}}';
        $mess->setRaw($raw);
        $ret = $push->PushSingleDevice($device_token, $mess, XingeApp::IOSENV_DEV);
        return $ret;
    }
...
//终端收到后通过intent.parseUri()来反序列化得到Intent
    function DemoPushSingleDeviceNotificationIntent()
    {
        ...
        $action = new Click();
        $action->setActionType(Click::TYPE_INTENT);
