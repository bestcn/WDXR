...
class Statistics extends Services
{


    static public function getStatisticsListPagintor($parameters, $numberPage,$where = null)
    {
        $conditions = '';$bind = [];
        if(!empty($parameters)) {
        ...
        $builder = Services::getStaticModelsManager()->createBuilder()
            ...
            ->andWhere("$where")
            ->from('Wdxr\Models\Entities\Statistics')
            ->orderBy('id desc');
