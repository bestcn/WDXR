...
class Api {
   ...
   /*
     ...
     * @param int $expireAfterSecs url will expire after this secconds.
     * @return array|mixed.
     */
    ...
    public function getDomainUrl($bucket, $fpath, $expireAfterSecs)
    {
        $fpath = $this->normalizerPath($fpath, false);
        $url_signature = '';
        if($expireAfterSecs !== false) {
            $expiration = time() + $expireAfterSecs;
            $signature = $this->auth->createReusableSignature($expiration, $bucket);
            $url_signature = "?sign=$signature";
        }
        ...
        return "$domain$fpath$url_signature";
    }
}
