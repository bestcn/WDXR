...
class Database extends Adapter
{
     ...
     * @return array
     */
    public function getResourceAccess($resource_name)
    {
        $accesses_name = [];
        $sql         = "SELECT `access_name` FROM {$this->resourcesAccesses} WHERE `RESOURCES_NAME` = '{$resource_name}'";
        foreach ($this->connection->fetchAll($sql, Db::FETCH_NUM) as $row) {
            $accesses_name[] = $row[0];
        }

        return $accesses_name;
    }

    public function getAllResourceAccess()
    {
        $list = [];
        $sql         = "SELECT `resources_name`, `access_name` FROM {$this->resourcesAccesses}";
        foreach ($this->connection->fetchAll($sql, Db::FETCH_NUM) as $key => $row) {
            $list[$key]['resource_name'] = $row[0];
            $list[$key]['access_name'] = $row[1];
        }
