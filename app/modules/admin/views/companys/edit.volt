    ...
    <div class="col-lg-12">
        <div class="tabs-container">
            <ul class="nav nav-tabs">
                <li class="active">{{ acl_button(['admin/companys/edit/'~id, '基本信息']) }}</li>
                <li class="">{{ acl_button(['admin/companys/edit_password/'~id~'/'~type, '修改企业密码']) }}</li>
                <li class="">{{ acl_button(['admin/companys/company_info/'~id~'/'~type, '详细信息']) }}</li>
                <li class="">{{ acl_button(['admin/companys/bill_info/'~id~'/'~type, '票据信息']) }}</li>
                <li class="">{{ acl_button(['admin/companys/report_info/'~id~'/'~type, '征信报告']) }}</li>
                    ...
                    <div class="panel-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-4">
                                <select id="top_category" name="top_category" class="form-control">
                                    {% for data in all_top%}
                                    <option  {% if top_category['code'] == data['code'] %}selected="selected"{% endif %} value="{{ data['code'] }}">{{ data['name'] }}</option>
                                    {% endfor %}
                                </select>
                            </div>
                            ...
                            <div class="col-sm-4">
                                <select id="sub_category" name="sub_category" class="form-control">
                                    {% for data2 in all_sub%}
                                        <option  {% if sub_category['code'] == data2['code'] %}selected="selected"{% endif %} value="{{ data2['code'] }}">{{ data2['name'] }}</option>
                                    {% endfor %}
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-4">
                                    <select  name="login_status" class="form-control">
                                        <option value= 0 {% if login_status == 0 %}selected="selected"{% endif %} >禁止登录</option>
                                        <option value= 1 {% if login_status == 1 %}selected="selected"{% endif %} >允许登录</option>
                                    </select>
                                </div>
                        ...
                        <div class="form-group">
                            <div class="col-sm-offset-2 col-sm-10">
                                {{ form.render('submit') }}
                                <button onclick="location='{{ url('admin/companys/goBack') }}';" class="btn btn-default" type="button">返回</button>
                            </div>
                        </div>
                    </div>
                    {{ end_form() }}
