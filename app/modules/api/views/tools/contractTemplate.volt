...
<head>
    ...
    <style>
        *{  margin: 0;  padding: 0;  font-family: SimSun, serif;  }
        ...
        .t-i{  text-indent:2em;  }
        .body-box{  padding: 35px;  min-width: 770px;  }
        .fz17{  font-size: 20px;  }
        ...
        .double-strong {  font-weight: bold;  text-decoration: underline;  text-decoration-style: double;  }
...
<div class="body-box">
    ...
    <div class="t-i qixian">
        乙方入驻“冀企管家”服务平台期限为 1 年，即 {{ start_time ? date('Y', start_time) : '' }} 年 {{ start_time ? date('m', start_time) : '' }} 月 {{ start_time ? date('d', start_time) : '' }} 日至 {{ end_time ? date('Y', end_time) : '' }} 年 {{ end_time ? date('m', end_time) : '' }} 月 {{ end_time ? date('d', end_time) : '' }} 日。
    </div>
    <div class="m-t25">
        三、服务费用及支付方式
    </div>
    <div class="t-i qixian">
        1、乙方向甲方支付年度服务费用总计人民币（大写） {{ money_chinese }} 元（￥{{ money }} 元）。
        企业等级为
    ...
    <div class="t-i qixian">
        2、本协议签订之日，乙方需向甲方支付年度服务费用人民币（大写） {{ money_chinese }} 元（￥{{ money }} 元）。
    ...
    <div class="t-i m-t25 qixian">本合同于 {{ verify_time ? date('Y', verify_time) : '' }} 年 {{ verify_time ? date('m', verify_time) : '' }} 月 {{ verify_time ? date('d', verify_time) : '' }} 日在 {{ sign_address }} 签订。</div>
    ...
    <div class="m-t300 ta-c bianhao">编号：{{ num }}</div>
</div>
</body>
</html>
